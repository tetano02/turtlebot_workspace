...
# Callback risultato
    def get_result_callback(self, future):
        ...
        # NOTA: La logica qui sotto gestisce se il goal completato era il paziente o il ritorno a casa.
        
        if not self.at_destination:
            # Arrivato al Paziente (Primo Goal completato)
            self.get_logger().info('✅ Arrivato alla destinazione (Paziente)!')
            msg = Bool()
            msg.data = False
            self.move_status_publisher.publish(msg)
            ...
            # Torna a casa
            self.get_logger().info('🔄 Inizio rientro a casa...')
            self.send_goal(self.home_x, self.home_y, self.home_theta)
        
        else:
            # Arrivato a Casa (Secondo Goal completato)
            self.get_logger().info('🏠 Arrivato a casa! In attesa di un nuovo comando vocale.')
            msg = Bool()
            msg.data = False
            self.move_status_publisher.publish(msg)
            self.at_destination = False # Reimposta per il prossimo comando vocale
            # NON eseguire rclpy.shutdown() qui, altrimenti il nodo si chiude e non ascolta più comandi.
